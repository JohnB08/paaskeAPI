 ...
 * @returns 
 */
const insertUserQuestionRelationship = async(userID: number, questionIdArray: number[])=>{
    try{
        const successArray = []
 ...
 * }
 */
export const createUserInDB = async(username: string, questionAmount: number = 14, excludedID: number[] = [22], insertAtEnd: boolean = true)=>{
    try{
        const data = await db.query(`
 ...
 * }
 */
export const getAnswer = async(userID: number, answer: string, questionId: number) =>{
    try{
        const data = await db.query(`
        SELECT question_id
        FROM paskequestions
